...
static void runServer(int port) {
    ...
    s.WaitForClient(); // Intercambia claves con el cliente
    s.StartChatLoop(); // Chat bidireccional en paralelo
}
...
static void runClient(const std::string& ip, int port) {
    ...
    c.ExchangeKeys();         // Intercambia claves con el servidor
    c.SendAESKeyEncrypted();  // Envía clave AES cifrada

    c.StartChatLoop();        // Chat bidireccional en paralelo
}
